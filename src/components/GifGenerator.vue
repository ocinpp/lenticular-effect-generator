  ...
  try {
    ...
    const resetProgressTimeout = () => {
      ...
      progressTimeout = setTimeout(() => {
        ...
        console.warn(`GIF encoding appears stuck. Time since last progress: ${timeSinceLastProgress}ms`)
    ...
    gif.on('progress', (p: number) => {
      ...
      generationStatus.value = `${t('gif.generating')} ${Math.round(progress.value)}%`

      console.log('GIF encoding progress:', encodingProgress + '%', 'Total:', Math.round(progress.value) + '%')
    ...
    const frameDelay = Math.max(100, (gifDuration.value * 1000) / totalFrames.value) // Minimum 100ms
    ...
    for (let i = 0; i < totalFrames.value; i++) {
      ...
      generationStatus.value = `${t('gif.renderingFrame')} ${i + 1}/${totalFrames.value}`
      ...
      if (normalizedTime <= 0.5) {
        tiltValue = -0.7 + (normalizedTime * 2) * 1.4
      } else {
        tiltValue = 0.7 - ((normalizedTime - 0.5) * 2) * 1.4
      }

      // Render frame
      const canvas = renderLenticularFrame(tiltValue, settings.width, settings.height)
...
const renderLenticularFrame = (tiltValue: number, width: number, height: number): HTMLCanvasElement => {
  const canvas = document.createElement('canvas')
  canvas.width = width
...
const renderLenticularEffect = (
  ...
  for (let strip = 0; strip < numStrips; strip++) {
    ...
    const stripIdx1 = Math.max(0, Math.min(Math.floor(stripImageSelector), images.length - 1))
    const stripIdx2 = Math.max(0, Math.min(Math.ceil(stripImageSelector), images.length - 1))
    ...
    if (images[stripIdx1] && images[stripIdx1].complete) {
      try {
        ...
        const sourceWidth = (actualStripWidth / width) * images[stripIdx1].width

        ctx.globalAlpha = 1 - stripMix
        ctx.drawImage(
          images[stripIdx1],
          sourceX, 0, sourceWidth, images[stripIdx1].height,
          x, 0, actualStripWidth, height
        )

        if (stripIdx2 !== stripIdx1 && images[stripIdx2] && images[stripIdx2].complete) {
          ctx.globalAlpha = stripMix
          const sourceX2 = (x / width) * images[stripIdx2].width
          const sourceWidth2 = (actualStripWidth / width) * images[stripIdx2].width

          ctx.drawImage(
            images[stripIdx2],
            sourceX2, 0, sourceWidth2, images[stripIdx2].height,
            x, 0, actualStripWidth, height
          )
        }
...
const createFallbackFrame = (ctx: CanvasRenderingContext2D, width: number, height: number, tiltValue: number) => {
...
<template>
  ...
  >
    <div class="bg-white rounded-xl p-4 sm:p-6 w-full max-w-md max-h-[95vh] sm:max-h-[90vh] overflow-y-auto">
      <!-- Header -->
      <div class="flex items-center justify-between mb-4 sm:mb-6">
        <h2 class="text-lg sm:text-xl font-bold text-gray-800">{{ t('gif.generateGif') }}</h2>
        <button
          v-if="!isGenerating"
          @click="closeModal"
      ...
      <div v-if="!isGenerating" class="space-y-4 mb-4 sm:mb-6">
        ...
        <div class="bg-blue-50 rounded-lg p-3">
          <div class="text-sm text-blue-800">
            ...
            <div>{{ t('gif.size') }}: {{ GIF_SETTINGS[gifQuality].width }}×{{ GIF_SETTINGS[gifQuality].height }}px</div>
            <div>{{ t('gif.duration') }}: {{ gifDuration }}{{ t('gif.seconds') }}</div>
            <div>{{ t('gif.frames') }}: {{ totalFrames }}</div>
            <div>{{ t('gif.estimatedSize') }}: {{ gifQuality === 'basic' ? '0.5-2MB' : '1-4MB' }}</div>
          </div>
        </div>
        ...
        <div class="bg-yellow-50 rounded-lg p-3">
          <div class="text-sm text-yellow-800">
            <div class="font-medium mb-1">📱 {{ t('gif.mobileOptimized') }}:</div>
      ...
      <div v-if="isGenerating" class="mb-4 sm:mb-6">
        <div class="text-center mb-4">
          <div class="text-base sm:text-lg font-semibold text-gray-800 mb-2">{{ generationStatus }}</div>
          <div class="text-sm text-gray-600">
            {{ currentFrame > 0 && progress < 50 ? t('gif.renderingFrame') + ' ' + currentFrame + '/' + totalFrames : '' }}
          </div>
        </div>
        ...
        <div class="mt-3 p-2 bg-blue-50 rounded-lg">
          <div class="text-xs text-blue-800 text-center">
            ...
            <div v-else-if="progress < 50">📦 {{ t('gif.preparingEncoding') }}</div>
