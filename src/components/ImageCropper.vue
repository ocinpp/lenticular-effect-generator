...
const handleMove = (event: MouseEvent | TouchEvent) => {
  ...
  if (isDragging.value) {
    const newX = Math.max(0, Math.min(imageData.value.width - cropArea.value.width, coords.clientX - dragStart.value.x))
    const newY = Math.max(0, Math.min(imageData.value.height - cropArea.value.height, coords.clientY - dragStart.value.y))
    ...
    if (resizeHandle.value === 'resize-se') {
      ...
      const newWidth = Math.max(100, Math.min(imageData.value.width - cropArea.value.x, cropArea.value.width + deltaX))
...
<template>
  <div ref="containerRef" class="h-full flex flex-col overflow-hidden">
    ...
    <div class="flex-1 flex items-center justify-center p-4 min-h-0 overflow-hidden">
      <div class="relative inline-block">
        ...
        >
          ...
          <div class="absolute -top-8 left-0 bg-blue-600 text-white text-xs px-2 py-1 rounded pointer-events-none">
            3:4 Portrait
          </div>
        </div>
        
        <!-- Dimmed Overlay -->
        <div class="absolute inset-0 bg-black/50 pointer-events-none" style="clip-path: polygon(0% 0%, 0% 100%, var(--crop-x) 100%, var(--crop-x) var(--crop-y), calc(var(--crop-x) + var(--crop-width)) var(--crop-y), calc(var(--crop-x) + var(--crop-width)) calc(var(--crop-y) + var(--crop-height)), var(--crop-x) calc(var(--crop-y) + var(--crop-height)), var(--crop-x) 100%, 100% 100%, 100% 0%)"
          :style="{
    ...
    <div class="flex-shrink-0 px-4 pb-4 bg-black/20 backdrop-blur-sm border-t border-white/10">
      <div class="bg-white/10 backdrop-blur-sm rounded-lg p-4">
        <div class="flex items-center justify-between mb-4">
          <div class="text-white text-sm">
            <p class="font-medium">Touch: Drag to move • Pinch corner to resize</p>
            <p class="text-slate-400">
              Crop size: {{ Math.round(cropArea.width / imageData.scale) }} × {{ Math.round(cropArea.height / imageData.scale) }}px (3:4 portrait)
            </p>
            <p class="text-blue-300 text-xs">Perfect aspect ratio for mobile viewing</p>
          </div>
        </div>
